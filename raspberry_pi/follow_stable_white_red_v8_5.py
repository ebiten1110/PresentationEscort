...
def main() -> None:
    ...
    print(
        "[FaceLostConfig] "
        f"GRACE={FACE_LOST_GRACE_SECONDS:.2f}s "
        "FACE=WHITE LOST=RED"
    )
        ...
        while True:
            ...
            # ==================================================
                ...
                if grace_active:
                    ...
                    # 候補回数と距離履歴は維持する。
                    # ただし最後の顔情報だけで新しい移動は開始しない。
                    print(
                        f"[Loop {loop_count:04d}] "
                        f"Face=GRACE "
                        f"lost={face_lost_seconds:.3f}s/"
                        f"{FACE_LOST_GRACE_SECONDS:.2f}s "
                        f"keepCandidate="
                        f"{body_start_candidate}:"
                        f"{body_start_count} "
                        f"keepDistanceCandidate="
                        f"{distance_candidate}:"
                        f"{distance_candidate_count} "
                        f"active={esp32.active_body} "
                        f"RGB={rgb_mode}"
                    )
